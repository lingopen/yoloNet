...
namespace yoloNetv2.Controls
{
    ...
    public static class VideoHelper
    {
        ...
        // 🔹 最后一帧图像流
        public static Stream? ImageStream => _bitmap?.ToStream();
        ...
        // 🔹 YOLO 推理
        private static YoloService _yolo = new YoloService();
        ...
        private static void OnPixelBufferArrivedAsync(PixelBufferScope bufferScope)
        {
            ...
            SKBitmap? localBitmap = null;

            try
            {
                localBitmap = SKBitmap.Decode(image);
                ...
                using (var pix = localBitmap.PeekPixels())
                {
                    ...
                    if (_session != null && Interlocked.CompareExchange(ref isInferencingFlag, 1, 0) == 0)
                    {
                        ...
                        _ = Task.Run(() =>
                        {
                            ...
                            var tensor = _yolo.CreateTensor(copy.Width, copy.Height);
                            _yolo.FillTensor(copy, tensor);
                            ...
                            try
                            {
                                ...
                                if (boxes.Any())
                                {
                                    ...
                                    _lastDetectedScore = $"检测耗时 | {_yoloStopwatch.ElapsedMilliseconds}ms";
                                }
                                else
                                {
                                    _lastDetectedFace = new Rect(0, 0, 0, 0);
                                    _lastDetectedScore = $"未识别到目标 | {_yoloStopwatch.ElapsedMilliseconds}ms";
                                }
                            }
                            catch (Exception ex)
                            {
        ...
        // 🔹 初始化摄像头
        public static void Init(int index = 0)
        {
            if (IsInit) return;
            ...
            if (Characteristics != null && UICanvas != null)
            {
                ...
                UICanvas.Height = Characteristics.Height;
            }
            IsInit = true;
        }
        // 🔹 启动摄像头 + 可选加载 ONNX 模型
        ...
        // 🔹 截图当前帧
        public static Stream? CapturePhoto()
        {
            if (_bitmap != null)
            {
                return _bitmap.ToStream();
            }
            return null;
        }
